...
fn test_idempotent_reserve_returns_same_port() {
    ...
    // First reservation
    let plan1 = ReservePlan::new(options.clone()).build_plan(&db).unwrap();
    ...
    let plan2 = ReservePlan::new(options.clone()).build_plan(&db).unwrap();
    let mut executor = PlanExecutor::new(&mut db);
    ...
    let plan3 = ReservePlan::new(options).build_plan(&db).unwrap();
    let mut executor = PlanExecutor::new(&mut db);
...
fn test_idempotent_reserve_updates_timestamp() {
    ...
    // Second reservation
    let plan2 = ReservePlan::new(options).build_plan(&db).unwrap();
    let mut executor = PlanExecutor::new(&mut db);
...
fn test_reserve_after_release_creates_new_reservation() {
    ...
    let plan2 = ReservePlan::new(reserve_opts2).build_plan(&db).unwrap();
